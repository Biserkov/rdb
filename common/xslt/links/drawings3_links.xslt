...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/02/xpath-functions" xmlns:xdt="http://www.w3.org/2005/02/xpath-datatypes">
	...
	<xsl:variable name="title"><xsl:if test="/book/@lang = 'en'">Vol </xsl:if><xsl:if test="/book/@lang = 'bg'">Том </xsl:if> <xsl:number format="I" value="/book/@id"/> - <xsl:if test="/book/@lang = 'en'">drawings</xsl:if><xsl:if test="/book/@lang = 'bg'">илюстрации</xsl:if></xsl:variable>
	<xsl:variable name="type">drawings</xsl:variable>
              ...
              <a>
					...
					<xsl:for-each select="species">
						...
						<tr>
							<td>
								<a>
									<xsl:attribute name="href"><xsl:value-of select="code"/>.html#draw</xsl:attribute>
