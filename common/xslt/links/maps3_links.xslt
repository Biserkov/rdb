...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/02/xpath-functions" xmlns:xdt="http://www.w3.org/2005/02/xpath-datatypes">
	...
	<xsl:variable name="title"><xsl:if test="/book/@lang = 'en'">Vol </xsl:if><xsl:if test="/book/@lang = 'bg'">Том </xsl:if> <xsl:number format="I" value="/book/@id"/> - <xsl:if test="/book/@lang = 'en'">distribution maps</xsl:if><xsl:if test="/book/@lang = 'bg'">карти на разпространението</xsl:if></xsl:variable>
	<xsl:variable name="type">maps</xsl:variable>
			...
			<body>
				...
				<table cellpadding="10">
					<tr>
						...
						<th>
              <a>
								<xsl:attribute name="href"><xsl:value-of select="$type"/>2.html</xsl:attribute>
								<xsl:if test="@id &lt; 3">
						...
						<th><xsl:if test="/book/@lang = 'en'">Degree of threat</xsl:if><xsl:if test="/book/@lang = 'bg'">Застрашеност</xsl:if></th>
					</tr>
					<xsl:for-each select="species">
						...
						<tr>
							<td>
								<a>
									<xsl:attribute name="href"><xsl:value-of select="code"/>.html#map</xsl:attribute>
